...
class CatBreedingRecipeProvider(registries: HolderLookup.Provider, output: RecipeOutput) :
  ...
  inner class Builder(result: ResourceKey<CatType>, parent1: ResourceKey<CatType>, parent2: ResourceKey<CatType>) {
    val lookup = registries.lookupOrThrow(CatRegistries.CAT_TYPE_REGISTRY)
    val result = lookup.getOrThrow(result)
    val parent1 = lookup.getOrThrow(parent1)
    val parent2 = lookup.getOrThrow(parent2)
    var chance = 100.0
    ...
    fun save(output: RecipeOutput, variant: Int) {
      val key = ResourceKey.create(
        Registries.RECIPE, 
        ProductiveCats.rl("cat_breeding/${result.value().type}_${variant}")
      )
      save(output, key)
    }
    fun save(output: RecipeOutput) = save(output, 0)
